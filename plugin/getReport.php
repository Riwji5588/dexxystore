...
if ($_POST) {
    if ($_POST['action'] == 'getreport') {
        if ($_POST['type'] == 1) {
            $select_claim = "SELECT * FROM data_claim WHERE confirm!=9 AND isDelete = 0 ORDER BY id DESC LIMIT 100";
            ...
            $select_claim = "SELECT * FROM data_claim_first WHERE confirm=0 AND isDelete = 0 ORDER BY id DESC";
            $claim_result = $hyper->connect->query($select_claim);
            $i = 0;
            $row = mysqli_num_rows($claim_result);
            if ($row > 0) {
