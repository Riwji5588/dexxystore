...
function sendNotify($from, $to, $type, $orderid)
{
    ...
    if ($type == "claim") { //when user send claim then admin will recept it
        for ($i = 0; $i < $row; $i++) {
            $msg = "{$from_user_data['username']} ทำการขอเคลมสินค้าออเดอร์ที่ {$orderid}"; // to admin
        ...
        $msg = "ออเดอร์ที่ {$orderid} ของคุณได้ถูกดำเนินการแล้ว กรุณาเช็คผลได้ที่ประวัติการซื้อของคุณได้เลย"; // to user
        $encode = base64_encode($msg);
